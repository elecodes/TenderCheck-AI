    ...
    <div className="w-full max-w-4xl mx-auto space-y-10 animate-fade-up">
      ...
      <div className="bg-white/30 dark:bg-[#1a1f24]/60 backdrop-blur-3xl rounded-[32px] shadow-[0_20px_50px_rgba(31,38,135,0.1)] border border-white/40 dark:border-white/10 p-10 transition-soft hover-lift hover:border-emerald-100/50 dark:hover:border-emerald-900/50">
        ...
        <div className="flex items-start space-x-6 mb-8">
            ...
            <h2 className={`${analysis.tenderTitle.length > 80 ? 'text-xl md:text-2xl' : 'text-3xl md:text-4xl'} font-serif font-medium text-brand-charcoal dark:text-white leading-[1.2]`}>
              {analysis.tenderTitle}
            </h2>
        </div>

        {/* Row 2: Metadata & Actions */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-t border-white/20 dark:border-white/10 pt-8">
            ...
            <div className="flex flex-wrap items-center gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">
                <span className="flex items-center bg-white/50 dark:bg-white/5 px-4 py-2 rounded-full border border-white/20 dark:border-white/5">
                ...
                <span className={`px-4 py-2 rounded-full border font-black ${
                    analysis.status === 'COMPLETED' ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-amber-50 dark:bg-amber-900/20 border-amber-100 dark:border-amber-500/20 text-amber-600 dark:text-amber-400'
                }`}>
            ...
            <div className="flex flex-wrap gap-3 flex-shrink-0">
              ...
              <button 
                onClick={onReset}
                className="flex items-center space-x-2 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 border border-gray-100 dark:border-white/10 rounded-xl transition-soft hover-lift"
              >
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-4">
            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-brand-charcoal dark:text-white flex items-center">
            ...
            {analysis.requirements && analysis.requirements.length > 0 ? (
                analysis.requirements.map((req, idx) => (
                    <div key={idx} className={`p-8 rounded-[28px] border bg-white/40 dark:bg-white/[0.03] backdrop-blur-md transition-soft hover-lift shadow-sm hover:shadow-md ${
                        ...
                        <p className="text-brand-charcoal dark:text-gray-300 text-lg font-sans font-medium leading-relaxed mb-6">{req.text}</p>
                        <div className="flex flex-wrap gap-2">
                            {req.keywords.map((kw, kIdx) => (
                                <span key={kIdx} className="text-[9px] font-black bg-white/50 dark:bg-white/5 px-3 py-1.5 rounded-lg border border-white/20 dark:border-white/5 text-brand-charcoal/60 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:border-emerald-200 dark:hover:border-emerald-800 transition-soft uppercase tracking-widest cursor-default">
                                    {kw}
                                </span>
                            ))}
                        </div>
                ...
                <div className="text-center p-16 bg-white/40 dark:bg-white/[0.02] rounded-3xl border border-dashed border-white/20 dark:border-white/10 text-brand-charcoal/40 dark:text-gray-400">
