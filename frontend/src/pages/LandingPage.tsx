  ...
  return (
    <div className="min-h-screen w-full bg-[#D3D0C2] bg-gradient-to-br from-[#E8E6DE] via-[#D3D0C2] to-[#B8C1B7] dark:bg-[#1a1f24] dark:bg-gradient-to-br dark:from-[#3a4450] dark:via-[#242b33] dark:to-[#1a1f24] text-[#2D312D] dark:text-white overflow-hidden relative font-sans isolation-auto transition-colors duration-500">
      ...
      <main className="pt-48 pb-48 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col items-center text-center relative z-10 animate-fade-up">
        ...
        <h1 className="text-6xl md:text-8xl font-serif font-medium mb-8 leading-[0.9] max-w-5xl text-[#2D312D] dark:text-white tracking-tight">
        ...
        <div className="w-full max-w-md relative group hover-lift">
            ...
            <div className="relative rounded-[32px] bg-white/30 dark:bg-white/[0.03] backdrop-blur-3xl border border-white/50 dark:border-white/10 shadow-[0_20px_50px_rgba(31,38,135,0.1)] p-12 flex flex-col items-center gap-8 overflow-hidden transition-soft">
                
                {/* Icon Box */}
                <div className="w-20 h-20 rounded-2xl bg-[#2D312D]/5 dark:bg-white/5 shadow-inner flex items-center justify-center mb-2 border border-black/5 dark:border-white/10 transition-soft group-hover:scale-110">
                    <UploadCloud className="w-8 h-8 text-[#C5A028] dark:text-[#D4AF37]" />
                </div>
                
                <div className="text-center space-y-3">
                    <h3 className="font-serif text-3xl text-[#2D312D] dark:text-white">Nuevo Análisis</h3>
                    <p className="text-[#2D312D] dark:text-gray-400 text-lg leading-snug max-w-[260px]">
                        Sube tus documentos PDF de licitación para verificación de cumplimiento impulsada por IA.
                    </p>
                </div>

                <Link to="/register" className="w-full py-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold tracking-widest uppercase shadow-xl shadow-emerald-600/20 transition-soft active:scale-[0.98] flex items-center justify-center mt-4 group-hover:shadow-2xl group-hover:shadow-emerald-600/30 border border-emerald-600/20">
                   Subir Licitación
                </Link>
            </div>
        </div>
        ...
        <section className="relative md:absolute bottom-0 w-full border-t border-black/5 dark:border-white/5 bg-[#2D312D]/5 dark:bg-black/40 backdrop-blur-md py-8 mt-12 md:mt-0 animate-fade-up" style={{ animationDelay: '0.4s' }}>
             <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-center items-center gap-12 md:gap-24 opacity-80">
                <div className="flex items-center gap-4 group">
                    <div className="p-3 rounded-full bg-black/5 dark:bg-white/5 group-hover:bg-black/10 dark:group-hover:bg-white/10 transition-colors border border-black/5 dark:border-white/5">
                        <UploadCloud className="w-6 h-6 text-[#2D312D] dark:text-gray-300" />
                    </div>
                    <div className="text-left">
                        <h4 className="font-serif font-medium text-[#2D312D] dark:text-gray-200">Extracción Inteligente</h4>
                        <p className="text-xs text-[#6B726B] dark:text-gray-500 uppercase tracking-wider">Análisis Automático de Requisitos</p>
                    </div>
                </div>
                <div className="flex items-center gap-4 group">
                    <div className="p-3 rounded-full bg-black/5 dark:bg-white/5 group-hover:bg-black/10 dark:group-hover:bg-white/10 transition-colors border border-black/5 dark:border-white/5">
                        <ShieldCheck className="w-6 h-6 text-[#2D312D] dark:text-gray-300" />
                    </div>
                    <div className="text-left">
                        <h4 className="font-serif font-medium text-[#2D312D] dark:text-gray-200">Verificación de Cumplimiento</h4>
                        <p className="text-xs text-[#6B726B] dark:text-gray-500 uppercase tracking-wider">Validación contra criterios</p>
                    </div>
                </div>
                <div className="flex items-center gap-4 group">
                    <div className="p-3 rounded-full bg-black/5 dark:bg-white/5 group-hover:bg-black/10 dark:group-hover:bg-white/10 transition-colors border border-black/5 dark:border-white/5">
                        <Zap className="w-6 h-6 text-[#2D312D] dark:text-gray-300" />
                    </div>
                    <div className="text-left">
                        <h4 className="font-serif font-medium text-[#2D312D] dark:text-gray-200">Análisis Instantáneo</h4>
