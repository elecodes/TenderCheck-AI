...
export class OllamaModelService implements ITenderAnalyzer {
  ...
  async analyze(text: string): Promise<TenderAnalysis> {
    try {
      ...
      const safeText = text.slice(0, 4000); // 4k chars to be safe
      console.log(`Analyzing text with Ollama (Model: ${this.model}). Length: ${safeText.length} chars...`);
      ...
      const findArray = (obj: any): any[] | null => {
        ...
        if (Array.isArray(obj.Requirements)) return obj.Requirements;
        if (Array.isArray(obj.technical_requirements)) return obj.technical_requirements;
      ...
      if (foundRequirements.length === 0 && typeof parsedRaw === 'object') {
          ...
          Object.keys(parsedRaw).forEach(key => {
             ...
             if (typeof val === 'string') {
                 foundRequirements.push({ text: `${key}: ${val}`, type: 'MANDATORY', keywords: [] });
             } else if (Array.isArray(val)) {
                 val.forEach(v => {
                    const text = typeof v === 'string' ? v : JSON.stringify(v);
                    foundRequirements.push({ text: `${key}: ${text}`, type: 'MANDATORY', keywords: [] });
                 });
             }
          });
      }
      ...
      const analysis: TenderAnalysis = {
        ...
        requirements: foundRequirements.map((req: any) => {
          ...
          let type = (req.type === 'OPTIONAL' || req.type === 'Optional') ? 'OPTIONAL' : 'MANDATORY';
          const optionalKeywords = ['should', 'could', 'desirable', 'valued', 'plus', 'optional', 'recommended', 'nice to have'];
